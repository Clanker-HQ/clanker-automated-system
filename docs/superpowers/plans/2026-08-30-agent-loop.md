...
describe("runReflection", () => {
  it("appends one reflection record per synthesised conclusion", async () => {
    // synthesise returns 2 → 2 records with kind "reflection".
  });

  it("supersedes by recency rather than rewriting an existing reflection", async () => {
    // An existing reflection on the same subject is still present afterwards;
    // the new one is appended alongside it. Never mutate in place — see the
    // spec's write-path rules.
  });

  it("passes both outcome records and run verdicts to the synthesiser", async () => {
    // The digest text handed to synthesise mentions the not-achieved verdict.
  });

  it("returns an empty array and writes nothing when there is no history", async () => {
    // Empty log + no runs → synthesise is never called.
  });

  it("never throws when the synthesiser rejects", async () => {
    // Returns [] — a failed reflection must not take down the scheduled job.
  });
});
```
