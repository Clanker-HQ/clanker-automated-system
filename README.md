...
`agent:<name>` instead of `discord:<id>`, and its priority is computed rather
than asked for — anywhere in 0-49, always below the human `!task` default of
...
work), importance, and recency combined into one score, then mapped onto the
queue's 0-49 band — always below the human `!task` default of 50, so a
self-queued task can never queue ahead of one a human actually asked for. A
task that finishes successfully can also propose its own **successors** — up
to a few follow-up tasks drawn from the run's own summary, capped by chain
depth and by a global rolling-day ceiling on agent-originated tasks
(`maxAgentTasksPerDay`, counted across every agent and chain at once, so a
wide shallow fan-out is bounded too) — one completed task can't cascade into
an unbounded chain of self-generated work. The log feeds forward as well as back: a
dispatched task's prompt ends with whatever the log already knows
about its subject (same domain, ranked by similarity and recency, with
reflections included whatever domain they were filed under, since a
synthesised conclusion is cross-cutting by construction), and a cron scout — which never goes through the dispatcher — gets
